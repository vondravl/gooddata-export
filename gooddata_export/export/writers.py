...
def export_visualizations(all_workspace_data, export_dir, config, db_name) -> None:
    ...
    with database_connection(db_name) as conn:
        ...
        # Drop old tables from v1.5.x that were consolidated into visualizations_references
        cursor = conn.cursor()
        cursor.execute("DROP TABLE IF EXISTS visualizations_metrics")
        cursor.execute("DROP TABLE IF EXISTS visualizations_attributes")
