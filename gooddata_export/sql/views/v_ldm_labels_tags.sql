...
-- dataset_id is derived from ldm_columns via attribute_id (normalized design)

DROP VIEW IF EXISTS v_ldm_labels_tags;

CREATE VIEW IF NOT EXISTS v_ldm_labels_tags AS
SELECT
    col.dataset_id,
    lbl.attribute_id,
    lbl.id AS label_id,
...
ORDER BY col.dataset_id, lbl.attribute_id, lbl.id, tags_json.value;
